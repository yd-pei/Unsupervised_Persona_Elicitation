...
def get_args():
    ...
    parser.add_argument(
        "--api_base", type=str, default="https://api.fireworks.ai/inference/v1/"
    )
    parser.add_argument(
        "--api_key", type=str, default=os.environ.get("FIREWORKS_API_KEY")
    )
...
I think this claim is """
        ...
        country_name = {"US": "United States", "FR": "France", "DE": "Germany"}[
            args.country
        ]
        ...
        for idx, i in enumerate(data):
            ...
            item["prompt"] = template.format(
                question=i["question"], country=country_name, option=i["option"]
            )
            item["uid"] = idx
...
def load_demonstrations(args):
    ...
    if not args.test_data_path:
        raise ValueError(
            "--test_data_path must be provided for many-shot, gold-label, or random testing"
        )
    ...
    for d in demo_data:
        if "prompt" not in d:
            d["prompt"] = template.format(
                question=d["question"], country=country_name, option=d["option"]
            )
...
def construct_messages(item, demonstrations, num_shots):
    ...
    if num_shots > 0 and demonstrations:
        candidates = [
            d for d in demonstrations if d.get("question") != item.get("question")
        ]
...
def main():
            ...
            try:
                ...
                if "429" in str(e) or "rate limit" in str(e).lower():
                    if attempt < max_retries - 1:
                        ...
                        print(
                            f"Rate limit hit for item {item['uid']}, retrying in {delay:.2f}s..."
                        )
                        time.sleep(delay)
                        continue
    ...
    output_file = (
        output_dir
        / f"instruct_eval_{args.testbed}_{args.country}_{args.test_mode}.json"
    )
